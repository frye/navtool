...
void main() {
  group('ENC Metadata Extraction Tests', () {
    ...
    testWidgets('should extract metadata from chart ID and filename', (tester) async {
      const testChartPath = 'test/fixtures/charts/noaa_enc/US5WA50M_harbor_elliott_bay.zip';
      ...
      expect(usageBand, equals(5), reason: 'Should extract usage band 5 from filename');
      
      print('Metadata extraction from filename:');
      print('  Cell ID: $cellId');
      print('  Usage Band: $usageBand');
      print('  Chart Type: ${usageBand == 5 ? 'Harbor' : usageBand == 3 ? 'Coastal' : 'Other'}');
    });
    ...
    testWidgets('should validate metadata consistency between chart types', (tester) async {
      ...
      if (harborMetadata.compilationScale != null && coastalMetadata.compilationScale != null) {
        expect(harborMetadata.compilationScale!, lessThan(coastalMetadata.compilationScale!),
    ...
    testWidgets('should validate coordinate and datum information', (tester) async {
    ...
    testWidgets('should demonstrate fixture availability status', (tester) async {
      print('Fixture availability status:');
      print('  Fixtures path: ${fixtures.fixturesPath}');
      print('  Primary chart (Harbor): ${fixtures.hasPrimaryChart ? 'Available' : 'Missing'}');
      print('  Secondary chart (Coastal): ${fixtures.hasSecondaryChart ? 'Available' : 'Missing'}');
