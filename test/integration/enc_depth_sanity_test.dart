...
void main() {
  group('ENC Depth Sanity Tests', () {
    testWidgets('should validate depth range checking logic', (tester) async {
      ...
      final testFeatures = [
        ...
        S57Feature(
          ...
          coordinates: [S57Coordinate(latitude: 47.5, longitude: -122.25)],
          attributes: {
        ...
        S57Feature(
          ...
          featureType: S57FeatureType.sounding,
          geometryType: S57GeometryType.point,
          coordinates: [S57Coordinate(latitude: 47.55, longitude: -122.25)],
          attributes: {
      ...
      for (final feature in testFeatures) {
        if (feature.featureType == S57FeatureType.depthArea) {
          print(
            '  DEPARE ${feature.recordId}: DRVAL1=${feature.attributes['DRVAL1']}, DRVAL2=${feature.attributes['DRVAL2']}',
          );
        } else if (feature.featureType == S57FeatureType.sounding) {
          print(
            '  SOUNDG ${feature.recordId}: VALSOU=${feature.attributes['VALSOU']}',
          );
        }
      }

      // Validate depth ranges
      final depthValidation = EncTestUtilities.validateDepthRanges(
        testParsedData,
      );
      ...
      print(
        '  Out of range percentage: ${depthValidation.outOfRangePercent.toStringAsFixed(1)}%',
      );
      print(
        '  Valid range: ${depthValidation.minDepth}m to ${depthValidation.maxDepth}m',
      );
      ...
      // Validate results
      expect(
        depthValidation.totalDepthFeatures,
        equals(4),
        reason: 'Should find 2 DEPARE + 2 SOUNDG features',
      );
      expect(
        depthValidation.outOfRangeCount,
        equals(3),
        reason:
            'Should find 3 out-of-range values (DRVAL1=-25, DRVAL2=150, VALSOU=200)',
      );
      expect(
        depthValidation.warnings.length,
        equals(3),
        reason: 'Should generate 3 warnings for out-of-range values',
      );
      expect(
        depthValidation.outOfRangePercent,
        closeTo(75.0, 1.0),
    ...
    testWidgets('should handle edge cases in depth validation', (tester) async {
      ...
      expect(
        validation.totalDepthFeatures,
        equals(4),
        reason: 'Should count all depth-related features (2 DEPARE + 2 SOUNDG)',
      );
    });

    testWidgets('should calculate depth distribution statistics', (
      tester,
    ) async {
      ...
      for (int i = 0; i < depthRanges.length; i++) {
        depthTestFeatures.add(
          S57Feature(
            recordId: i + 1,
            featureType: S57FeatureType.depthArea,
            geometryType: S57GeometryType.area,
            coordinates: [
              S57Coordinate(latitude: 47.5, longitude: -122.3 + i * 0.01),
            ],
            attributes: {
      ...
      for (int i = 0; i < soundingDepths.length; i++) {
        depthTestFeatures.add(
          S57Feature(
            ...
            geometryType: S57GeometryType.point,
            coordinates: [
              S57Coordinate(latitude: 47.5, longitude: -122.25 + i * 0.01),
            ],
            attributes: {'VALSOU': soundingDepths[i]},
          ),
        );
      }
      ...
      print(
        '  Valid percentage: ${(100.0 - validation.outOfRangePercent).toStringAsFixed(1)}%',
      );
    ...
    testWidgets(
      'should demonstrate depth sanity checking for different chart types',
      (tester) async {
...
class MockSpatialIndex implements SpatialIndex {
  ...
  @override
  List<S57Feature> queryNavigationAids() => _features
      .where(
        (f) => [
          S57FeatureType.lighthouse,
          S57FeatureType.beacon,
          S57FeatureType.buoy,
        ].contains(f.featureType),
      )
      .toList();

  @override
  List<S57Feature> queryDepthFeatures() => _features
      .where(
        (f) => [
  ...
  S57Bounds? calculateBounds() =>
      S57Bounds(north: 48, south: 47, east: -122, west: -123);
}
