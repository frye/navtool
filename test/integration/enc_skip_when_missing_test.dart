...
void main() {
  group('ENC Skip When Missing Tests', () {
    ...
    testWidgets('should detect empty fixtures directory', (tester) async {
      ...
      if (fixtures.found) {
        ...
        if (fixtures.hasSecondaryChart) {
          print('  ✓ Secondary chart available: ${fixtures.secondaryChartPath}');
        }
      } else {
    ...
    testWidgets('should detect partial fixtures (only one chart available)', (tester) async {
      final fixtures = EncTestUtilities.discoverFixtures();
    ...
    testWidgets('should use default path when environment variable not set', (tester) async {
      final fixtures = EncTestUtilities.discoverFixtures();
      ...
      if (!hasEnvVar) {
        expect(fixtures.fixturesPath, equals('test/fixtures/charts/noaa_enc'), 
        ...
        print('Using environment variable path: ${Platform.environment['NOAA_ENC_FIXTURES']}');
      }
    });
    
    testWidgets('should demonstrate proper skip pattern in tests', (tester) async {
      final fixtures = EncTestUtilities.discoverFixtures();
    ...
    testWidgets('should provide helpful skip messages', (tester) async {
      ...
      if (!fixtures.hasAnyFixtures) {
        const expectedMessage = 'No NOAA ENC fixtures – set NOAA_ENC_FIXTURES to enable.';
        
        print('Skip message validation:');
        print('  Standard message: "$expectedMessage"');
        
        // Validate message is helpful
        expect(expectedMessage, contains('NOAA_ENC_FIXTURES'), 
            reason: 'Skip message should mention environment variable');
        expect(expectedMessage, contains('No NOAA ENC fixtures'), 
            reason: 'Skip message should clearly state what is missing');
        expect(expectedMessage.length, lessThan(100), 
      ...
      if (!fixtures.hasSecondaryChart) {
        ...
        expect(secondaryMessage, contains('secondary'), 
    ...
    testWidgets('should handle fixture discovery errors gracefully', (tester) async {
    ...
    testWidgets('should document fixture requirements', (tester) async {
      ...
      print('');
      print('Chart IDs:');
      print('  Primary: ${EncTestUtilities.primaryChartId} (Usage Band 5 - Harbor)');
      print('  Secondary: ${EncTestUtilities.secondaryChartId} (Usage Band 3 - Coastal)');
      print('');
      print('Default Path:');
