...
void main() {
  group('Washington Chart Discovery Issue #129 - Integration Fix', () {
    ...
    test('should demonstrate and fix the Washington chart discovery cache issue', () async {
      ...
      logger.info(
        'TESTING: Searching for charts in Washington state bounds',
        context: 'WA.ChartDiscovery',
      );
      ...
      final foundBefore = await storageService.getChartsInBounds(
        washingtonBounds,
      );
      logger.warning(
        'ISSUE CONFIRMED: Found ${foundBefore.length} charts. Invalid legacy charts should not be discovered due to zero bounds.',
        context: 'WA.ChartDiscovery',
      );
      ...
      expect(
        foundIdsBefore.contains('US1WC01M'),
        isFalse,
        reason: 'Invalid bounded chart US1WC01M should not be discovered',
      );
      expect(
        foundIdsBefore.contains('US1WC04M'),
        isFalse,
        reason: 'Invalid bounded chart US1WC04M should not be discovered',
      );
      ...
      logger.info(
        'ANALYSIS: Found $invalidCount charts with invalid bounds (0,0,0,0)',
        context: 'WA.ChartDiscovery',
      );
      ...
      expect(
        invalidCount,
        equals(2),
        reason: 'Should detect exactly 2 charts with invalid bounds',
      );

      final clearedCount = await storageService.clearChartsWithInvalidBounds();
      logger.info(
        'CACHE INVALIDATION: Cleared $clearedCount charts with invalid bounds',
        context: 'WA.ChartDiscovery',
      );
      ...
      expect(
        clearedCount,
        equals(2),
        reason: 'Should clear exactly 2 problematic charts',
      );
      ...
      logger.info(
        'VERIFICATION: Searching for charts in Washington state bounds again',
        context: 'WA.ChartDiscovery',
      );

      final foundAfter = await storageService.getChartsInBounds(
        washingtonBounds,
      );
      logger.info(
        'ISSUE RESOLVED: Found ${foundAfter.length} charts after re-caching',
        context: 'WA.ChartDiscovery',
      );

      final foundIdsAfter = foundAfter.map((c) => c.id).toSet();
      expect(
        foundIdsAfter.containsAll({'US1WC01M', 'US1WC04M'}),
        isTrue,
        reason: 'Both corrected Washington charts should now be discovered',
      );

      // STEP 6: Verify cache is now clean
      final remainingInvalidCount = await storageService
          .countChartsWithInvalidBounds();
      logger.info(
        'FINAL STATE: $remainingInvalidCount charts with invalid bounds remain',
        context: 'WA.ChartDiscovery',
      );
      ...
      expect(
        ...
        equals(0),
        reason: 'No charts with invalid bounds should remain after fix',
      );

      logger.info(
        'SUCCESS: Washington chart discovery issue #129 resolved',
        context: 'WA.ChartDiscovery',
      );
      logger.info(
        'SUMMARY: cleared $clearedCount invalid charts; discovery returns ${foundAfter.length}; cache invalidation confirmed',
        context: 'WA.ChartDiscovery',
      );
    });
  });
