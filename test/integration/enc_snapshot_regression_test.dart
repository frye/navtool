...
void main() {
  group('ENC Snapshot Regression Tests', () {
    testWidgets('should demonstrate snapshot loading and comparison', (
      tester,
    ) async {
      ...
      print(
        '  Features out of tolerance: ${comparison.featuresOutOfTolerance}',
      );
      ...
      for (final result in comparison.results.values) {
        ...
        print(
          '  ${result.featureType}: ${result.actualCount} vs ${result.expectedCount} '
    ...
    testWidgets('should handle new feature types with warnings', (
      tester,
    ) async {
      const testSnapshot = EncSnapshot(
        ...
        edition: 1,
        update: 0,
        featureFrequency: {'DEPARE': 100, 'COALNE': 10},
      );
      ...
      expect(
        comparison.warnings.any((w) => w.contains('WRECKS')),
        isTrue,
        reason: 'Should specifically warn about WRECKS',
      );
      expect(
        comparison.warnings.any((w) => w.contains('LIGHTS')),
        isTrue,
        reason: 'Should specifically warn about LIGHTS',
      );
    });

    testWidgets('should demonstrate snapshot generation process', (
      tester,
    ) async {
      ...
      if (EncTestUtilities.isSnapshotGenerationAllowed) {
        ...
        final testSnapshotFile = File(
          'test/fixtures/golden/generated_test_freq.json',
        );
        ...
        final loadedSnapshot = await EncTestUtilities.loadSnapshot(
          'GENERATED_TEST',
        );
    ...
    testWidgets('should handle missing snapshots appropriately', (
      tester,
    ) async {
      // Try to load a non-existent snapshot
      final missingSnapshot = await EncTestUtilities.loadSnapshot(
        'NONEXISTENT_CHART',
      );
