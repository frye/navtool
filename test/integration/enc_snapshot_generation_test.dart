  ...
  group('ENC Snapshot Generation Tests', () {
    testWidgets('should demonstrate snapshot generation workflow', (tester) async {
      if (!EncTestUtilities.isSnapshotGenerationAllowed) {
        print('Skipping snapshot generation tests - set ALLOW_SNAPSHOT_GEN=1 to enable');
        return;
      }
      ...
      // Generate snapshot
      await EncTestUtilities.generateSnapshot(testChartId, testMetadata, testFrequencies);
      ...
      expect(content, isNotEmpty, 
          reason: 'Snapshot file should not be empty');
      expect(content.contains(testChartId), isTrue, 
    ...
    testWidgets('should create properly formatted JSON output', (tester) async {
      ...
      expect(content.contains('  '), isTrue, 
          reason: 'JSON should be indented');
    ...
    testWidgets('should handle directory creation if needed', (tester) async {
      if (!EncTestUtilities.isSnapshotGenerationAllowed) {
        print('Skipping directory creation test - set ALLOW_SNAPSHOT_GEN=1 to enable');
        return;
      }
      ...
      await EncTestUtilities.generateSnapshot('DIR_TEST', testMetadata, testFrequencies);
    ...
    testWidgets('should demonstrate snapshot generation disabled behavior', (tester) async {
      if (EncTestUtilities.isSnapshotGenerationAllowed) {
        print('Skipping disabled behavior test - ALLOW_SNAPSHOT_GEN is currently enabled');
        return;
      }
      ...
      print('Snapshots should only be generated during development/maintenance');
