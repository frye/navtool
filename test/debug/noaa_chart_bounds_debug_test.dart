...
void main() {
  group('NOAA Chart Bounds Debug Tests', () {
    ...
    test(
      'should fetch real NOAA chart catalog and analyze bounds for Washington coverage',
      () async {
        ...
        for (final chart in charts) {
          ...
          print(
            '  State: ${chart.state}, Type: ${chart.type}, Scale: ${chart.scale}',
          );

          // Check for invalid bounds
          if (chart.bounds.north == 0 &&
              chart.bounds.south == 0 &&
              chart.bounds.east == 0 &&
              chart.bounds.west == 0) {
          ...
          final intersectsWashington = _boundsIntersect(
            chart.bounds,
            washingtonBounds,
          );
          ...
          if (chart.id.contains('WC') ||
              chart.title.toLowerCase().contains('washington') ||
              chart.title.toLowerCase().contains('puget') ||
              chart.title.toLowerCase().contains('columbia')) {
        ...
        print('=== SUMMARY ===');
        print(
          'Charts with invalid bounds (0,0,0,0): ${chartsWithInvalidBounds.length}',
        );
        ...
        for (final chartId in targetCharts) {
          final chart = charts.firstWhere(
            (c) => c.id == chartId,
            orElse: () => throw Exception('Chart $chartId not found'),
          );
          ...
          print(
            '  Should cover Washington coordinates (47.6062, -122.3321): ${_pointInBounds(47.6062, -122.3321, chart.bounds)}',
          );
          print(
            '  Intersects Washington state: ${_boundsIntersect(chart.bounds, washingtonBounds)}',
          );
          print('');
        }
        ...
        expect(
          charts.length,
          greaterThan(0),
          reason: 'Should fetch some charts from NOAA',
        );
        expect(
          chartsWithInvalidBounds.length,
          equals(0),
          reason: 'Current NOAA data should have valid bounds',
        );

        // This test documents what we found - might fail if NOAA data doesn't cover Washington
        print(
          '🔍 DIAGNOSTIC COMPLETE - Check output above for Washington chart coverage analysis',
        );
      },
