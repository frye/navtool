  ...
  group('Chart Bounds Analysis', () {
    test('should analyze typical chart bounds patterns', () {
      ...
      try {
        ...
        print(
          '   ✅ Created: N:${bounds1.north} S:${bounds1.south} E:${bounds1.east} W:${bounds1.west}',
        );
      ...
      try {
        final bounds3 = GeographicBounds(
          north: 47.6062,
          south: 47.6062,
          east: -122.3321,
          west: -122.3321,
        ...
        print(
          '   ✅ Created: N:${bounds3.north} S:${bounds3.south} E:${bounds3.east} W:${bounds3.west}',
        );
      ...
      print(
        'Washington state: N:${washingtonBounds.north} S:${washingtonBounds.south} E:${washingtonBounds.east} W:${washingtonBounds.west}',
      );
      print('Area: ${_calculateArea(washingtonBounds)}');

      // Typical chart bounds in Washington waters
      final pugetsoundBounds = GeographicBounds(
        north: 48.5,
        south: 47.0,
        east: -122.0,
        west: -123.5,
      ...
      print(
        'Puget Sound area: N:${pugetsoundBounds.north} S:${pugetsoundBounds.south} E:${pugetsoundBounds.east} W:${pugetsoundBounds.west}',
      );
      print('Area: ${_calculateArea(pugetsoundBounds)}');

      print('\\n🎯 CONCLUSION:');
      print(
        'Charts with invalid bounds (0,0,0,0) or point coordinates will not intersect with state polygons.',
      );
      print(
        'This explains why 18 charts are cached but 0 charts are found for Washington spatial intersection.',
      );
      print(
        'The issue is likely that NOAA API is returning charts with placeholder/invalid geometry data.',
      );
    });
    ...
    test('should demonstrate the spatial intersection threshold', () {
      ...
      for (final chart in charts) {
        ...
        final coverage = _calculateCoverageEstimate(
          washingtonBounds,
          chartBounds,
        );
        ...
        print('\\n📋 ${chart['name']}:');
        print(
          '   Bounds: N:${chartBounds.north} S:${chartBounds.south} E:${chartBounds.east} W:${chartBounds.west}',
        );
      ...
      print('\\n💡 INSIGHT:');
      print(
        "Even valid charts might fail if they're too small relative to the state area.",
      );
      print(
        'Consider lowering the threshold for detailed harbor/approach charts.',
      );
    });
  });
}
...
double _calculateCoverageEstimate(
  GeographicBounds stateBounds,
  GeographicBounds chartBounds,
) {
  ...
  if (intersectionNorth <= intersectionSouth ||
