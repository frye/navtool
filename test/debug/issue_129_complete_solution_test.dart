...
void main() {
  group('Issue #129 Complete Solution Tests', () {
    ...
    test('Issue #129: Complete Washington chart discovery solution', () async {
      ...
      final oldWashingtonCharts = [
        Chart(
          ...
          scale: 80000,
          bounds: GeographicBounds(north: 0, south: 0, east: 0, west: 0), // Invalid bounds
      ...
      expect(invalidCountBefore, equals(2), reason: 'Should detect old charts with invalid bounds');
      ...
      expect(clearedCount, equals(2), reason: 'Should clear all charts with invalid bounds');
      ...
      expect(invalidCountAfter, equals(0), reason: 'Should have no invalid bounds after clearing');
      ...
      final realisticWashingtonCharts = [
        ...
        Chart(
          ...
          bounds: GeographicBounds(
            north: 47.0, south: 45.5, east: -123.0, west: -124.8 // Covers WA/OR border
          ),
        ...
        Chart(
          ...
          scale: 25000,
          bounds: GeographicBounds(
            north: 47.8, south: 47.0, east: -122.0, west: -123.0 // Covers Seattle area
          ),
          lastUpdate: DateTime.now(),
          state: 'Washington',
          type: ChartType.harbor,
        ...
        Chart(
          ...
          scale: 50000,
          bounds: GeographicBounds(
            north: 48.5, south: 47.5, east: -122.5, west: -124.8 // Covers northern WA waters
          ),
          lastUpdate: DateTime.now(),
          state: 'Washington',
        ...
        Chart(
          ...
          scale: 500000,
          bounds: GeographicBounds(
            north: 49.0, south: 45.0, east: -116.0, west: -125.0 // Covers entire WA coast
          ),
          lastUpdate: DateTime.now(),
          state: 'Washington',
      ...
      print('Added ${realisticWashingtonCharts.length} realistic Washington charts');
      print('');
      ...
      expect(chartsInWashington.length, equals(4), 
             reason: 'Should find all 4 realistic Washington charts');
      
      // Test Seattle area specifically
      final seattleBounds = GeographicBounds(
        north: 47.7, south: 47.5, east: -122.2, west: -122.4
      );
      final chartsAroundSeattle = await storageService.getChartsInBounds(seattleBounds);
      ...
      expect(chartsAroundSeattle.length, greaterThan(0), 
      ...
      expect(staleCount, equals(1), reason: 'Should detect the stale cached chart');
      ...
      expect(fixedCount, equals(1), reason: 'Should clear the stale cached chart');
      
      // Verify Washington charts are still available (valid ones weren't affected)
      final chartsAfterFix = await storageService.getChartsInBounds(washingtonBounds);
      expect(chartsAfterFix.length, equals(4), 
      ...
      print('✅ Cache invalidation: Clears charts with invalid bounds (0,0,0,0)');
      print('✅ Spatial intersection: Finds charts that intersect Washington bounds');
      print('✅ Data completeness: Realistic Washington chart coverage provided');
    ...
    test('Issue #129: Verify performance of cache invalidation at scale', () async {
      ...
      print('Creating $totalCharts charts ($invalidCharts with invalid bounds)...');
      ...
      expect(invalidCount, equals(invalidCharts), 
             reason: 'Should correctly count invalid charts at scale');
      
      // Clear invalid charts
      final clearedCount = await storageService.clearChartsWithInvalidBounds();
      expect(clearedCount, equals(invalidCharts), 
      ...
      print('- Throughput: ${(totalCharts / stopwatch.elapsedMilliseconds * 1000).round()} charts/second');
      ...
      expect(remainingCharts.length, equals(totalCharts - invalidCharts),
