...
void main() {
  group('NOAA Endpoint Configuration Verification', () {
    test('should verify application is using PRODUCTION NOAA endpoints', () async {
      ...
      expect(
        dio.options.baseUrl,
        equals('https://charts.noaa.gov'),
        reason: 'Application should use PRODUCTION NOAA base URL',
      );

      expect(
        NoaaApiClientImpl.catalogEndpoint,
        equals(
      ...
      expect(
        NoaaApiClientImpl.chartDownloadBase,
      ...
      print(
        'The application IS using real NOAA production endpoints, not test endpoints!',
      );

      // Act - Try to make a real request to see what data we get
      try {
        print('\\n🚀 TESTING REAL PRODUCTION API CALL...');
        final catalogData = await noaaClient.fetchChartCatalog(
          filters: {
        ...
        if (responseJson.contains('features')) {
          ...
          if (decodedResponse.contains('US1WC') ||
              decodedResponse.contains('US5WA') ||
              decodedResponse.contains('US3WA')) {
            print(
              '🗺️  Found West Coast/Washington charts in production data!',
            );
          } else {
            print(
              '⚠️  Limited Washington charts in current production response',
            );
          }
          ...
          if (chartCount > 15) {
            print(
              '✅ PRODUCTION dataset is larger than test dataset (15 charts)',
            );
          } else {
            print(
              '⚠️  Production response has limited charts - possible filtering or availability issue',
            );
          }
        } else {
        ...
        print(
          'This could indicate network issues, not endpoint configuration problems',
        );
      }

      print('\\n🎯 CONCLUSION:');
      print(
        'The NavTool application IS configured to use PRODUCTION NOAA endpoints:',
      );
      print('- Base URL: https://charts.noaa.gov (PRODUCTION)');
      print(
        '- Catalog: https://gis.charttools.noaa.gov/.../MapServer/0/query (PRODUCTION)',
      );
    ...
    test('should confirm there are NO test endpoint configurations', () {
      ...
      expect(
        catalogEndpoint,
        isNot(contains('test')),
        reason: 'Should not contain "test"',
      );
      expect(
        catalogEndpoint,
        isNot(contains('staging')),
        reason: 'Should not contain "staging"',
      );
      expect(
        catalogEndpoint,
        isNot(contains('dev')),
        reason: 'Should not contain "dev"',
      );
      expect(
        catalogEndpoint,
        isNot(contains('sandbox')),
        reason: 'Should not contain "sandbox"',
      );
    ...
    test(
      'should analyze why Washington charts might not appear despite production API',
      () {
        ...
        print(
          '   - NOAA production API might have limited West Coast coverage',
        );
        ...
        print(
          '- Verify our Issue #129 cache invalidation is actually running in the app',
        );
