...
Future<void> inspectNoaaChartBounds() async {
  ...
  try {
    final url =
        'https://gis.charttools.noaa.gov/arcgis/rest/services/encdirect/enc_coverage/MapServer/0/query?where=1%3D1&outFields=*&f=json&returnGeometry=true&resultRecordCount=1000';
        ...
        if (geometry != null) {
          ...
          if (rings != null && rings.isNotEmpty) {
            ...
            if (firstRing.isNotEmpty) {
              ...
              print(
                '  Calculated bounds: ($minLon, $minLat) to ($maxLon, $maxLat)',
              );
              ...
              if (maxLon >= washingtonMinLon &&
                  minLon <= washingtonMaxLon &&
                  maxLat >= washingtonMinLat &&
                  minLat <= washingtonMaxLat) {
