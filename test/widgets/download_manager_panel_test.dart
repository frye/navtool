...
class FakeDownloadService implements DownloadService {
  ...
  Future<BatchDownloadProgress> getBatchProgress(String batchId) async =>
      BatchDownloadProgress(
        batchId: batchId,
        status: BatchDownloadStatus.completed,
        totalCharts: 0,
        completedCharts: 0,
        failedCharts: 0,
        overallProgress: 0,
        lastUpdated: DateTime.now(),
        failedChartIds: const [],
      );
  @override
...
void main() {
  ...
  testWidgets('DownloadManagerPanel renders sections and queue position', (
    tester,
  ) async {
    ...
    final queued = [
      build(id: 'B456', status: DownloadStatus.queued),
      build(id: 'C789', status: DownloadStatus.queued),
    ];
    final completed = [
      build(id: 'D000', status: DownloadStatus.completed, prog: 1.0),
    ];
    final failed = [
      build(
        id: 'E999',
    ...
    final downloads = <String, DownloadProgress>{
      for (final d in [...active, ...queued, ...completed, ...failed])
        d.chartId: d,
    };
