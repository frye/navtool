  ...
  group('S57BackwardCompatibilityAdapter', () {
    ...
    group('legacyToAcronym', () {
      test('should map navigation aids correctly', () {
        expect(
          S57BackwardCompatibilityAdapter.legacyToAcronym(
    ...
    group('legacyToObjectClass', () {
      test(
        'should return object class for legacy types with catalog lookup',
        () {
          final objectClass =
              S57BackwardCompatibilityAdapter.legacyToObjectClass(
                S57FeatureType.depthArea,
                catalog,
              );
    ...
    group('hasOfficialEquivalent', () {
      test('should identify types with official equivalents', () {
        expect(
          S57BackwardCompatibilityAdapter.hasOfficialEquivalent(
            S57FeatureType.depthArea,
          ),
          isTrue,
        );
    ...
    group('getMappingStats', () {
      test('should return correct mapping statistics', () {
        ...
        expect(
          stats['total_legacy_types'],
          equals(S57FeatureType.values.length),
        );
    ...
    group('round-trip conversion', () {
      test('should maintain consistency in round-trip conversions', () {
        ...
        for (final legacyType in testTypes) {
          final acronym = S57BackwardCompatibilityAdapter.legacyToAcronym(
            legacyType,
          );
          final backToLegacy = S57BackwardCompatibilityAdapter.acronymToLegacy(
            acronym,
          );
          ...
          expect(
            backToLegacy,
            isNot(equals(S57FeatureType.unknown)),
            reason:
                'Round-trip conversion failed for $legacyType -> $acronym -> $backToLegacy',
          );
        }
      });
    });
