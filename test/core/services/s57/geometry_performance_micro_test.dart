  ...
  group('S57 Geometry Assembly Performance', () {
    test('micro-benchmark: assembly time for 100 features', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        store.addEdge(S57Edge(
          id: i,
          nodes: List.generate(4, (j) => S57Node(
            id: i * 10 + j,
            x: (i % 10).toDouble() + j,
            y: (i ~/ 10).toDouble() + j,
          )),
      ...
      final pointPointers = List.generate(33, (i) => [
        S57SpatialPointer(refId: i * 10, isEdge: false, reverse: false),
      ]);
      
      final linePointers = List.generate(34, (i) => [
        S57SpatialPointer(refId: i, isEdge: true, reverse: false),
      ]);
      
      final polygonPointers = List.generate(33, (i) => [
        S57SpatialPointer(refId: i, isEdge: true, reverse: false),
      ...
      final featuresPerMs = 100 / math.max(elapsedMs, 1); // Avoid division by zero
      ...
      expect(featuresPerMs, greaterThan(0.1),
      ...
      expect(polygonCount, greaterThanOrEqualTo(0)); // Polygons depend on edge connectivity
    });
    ...
    test('micro-benchmark: large polygon assembly', () {
      ...
      final pointers = List.generate(numNodes, (i) => 
        S57SpatialPointer(refId: i, isEdge: true, reverse: false));
