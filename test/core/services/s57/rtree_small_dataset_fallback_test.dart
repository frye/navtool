...
void main() {
  group('R-tree Small Dataset Fallback', () {
    ...
    test('should handle exactly threshold-sized datasets', () {
      final thresholdFeatures = _createTestFeatures(
        200,
    ...
    test('should handle single feature datasets', () {
      ...
      final pointResults = index.queryPoint(
        47.65,
        -122.35,
        radiusDegrees: 0.01,
      );
    ...
    test(
      ...
      () {
        ...
        // Test point query
        final linearPointResults = linearIndex.queryPoint(
          47.65,
          -122.35,
          radiusDegrees: 0.02,
        );
        final rtreePointResults = rtreeIndex.queryPoint(
          47.65,
          -122.35,
          radiusDegrees: 0.02,
        );
        ...
        for (int i = 0; i < linearPointResults.length; i++) {
          expect(
            rtreePointResults[i].recordId,
            equals(linearPointResults[i].recordId),
          );
        }
      },
    );
