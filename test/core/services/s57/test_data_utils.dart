...
/// Create valid S-57 test data with custom datum codes for testing datum validation
List<int> createValidS57TestDataWithCustomDatum({
  required String horizontalDatum,
  required String verticalDatum,
  required String soundingDatum,
}) {
  // Create a minimal but valid S-57 ISO 8211 record structure with custom datum codes
  final data = <int>[];
  ...
  // Add HDAT (Horizontal Datum) - exactly 4 bytes
  final hdatBytes = _padToFourBytes(horizontalDatum);
  data.addAll(hdatBytes);
  
  // Add VDAT (Vertical Datum) - exactly 4 bytes  
  final vdatBytes = _padToFourBytes(verticalDatum);
  data.addAll(vdatBytes);
  
  // Add SDAT (Sounding Datum) - exactly 4 bytes
  final sdatBytes = _padToFourBytes(soundingDatum);
  data.addAll(sdatBytes);
  ...
  // Add COMF (Coordinate Multiplication Factor) - 4 bytes
  addBinaryDouble(data, 10000000.0);
  
  // Add SOMF (Sounding Multiplication Factor) - 4 bytes
  addBinaryDouble(data, 10.0);
  ...
  // SG2D field data (2D Coordinates) - starts at 245+313=558
  final customComf = 10000000.0;
  final lat = (47.64 * customComf).round(); // Convert to S-57 coordinate units using COMF
  final lon = ((-122.34) * customComf).round();
  addBinaryInt(data, lon, 4); // X coordinate (longitude)
  addBinaryInt(data, lat, 4); // Y coordinate (latitude)
  addBinaryInt(data, lon + (1000 * customComf / 10000000).round(), 4); // Second point X
...
/// Helper function to pad datum strings to exactly 4 bytes
List<int> _padToFourBytes(String str) {
  final bytes = ascii.encode(str);
  if (bytes.length >= 4) {
    return bytes.sublist(0, 4); // Truncate to 4 bytes
  } else {
    // Pad with zeros to 4 bytes
    return [...bytes, ...List.filled(4 - bytes.length, 0)];
  }
}