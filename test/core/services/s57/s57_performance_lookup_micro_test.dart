  ...
  group('S57ObjectCatalog Performance', () {
    ...
    test('repeated lookups should complete within reasonable time', () {
      ...
      expect(
        ...
        reason:
            '$lookupCount code lookups took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
    ...
    test('repeated acronym lookups should complete within reasonable time', () {
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '$lookupCount acronym lookups took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
    ...
    test('attribute decoding should complete within reasonable time', () {
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '$decodeCount attribute decodings took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
    ...
    test('mixed operations should maintain O(1) performance', () {
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '${operationCount * 4} mixed operations took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });

    test('unknown lookups should not significantly impact performance', () {
      const int lookupCount = 1000;
      const int maxMilliseconds =
          200; // 0.2 second threshold (adjusted for warning overhead)
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '${lookupCount * 3} repeated unknown lookups took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
    ...
    test('catalog initialization overhead should be minimal', () {
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '10 catalog initializations took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
    ...
    test('large catalog should maintain good performance', () {
      ...
      final largeCatalog = S57ObjectCatalog.fromObjectClasses(
        largeObjectClasses,
      );
      ...
      expect(
        ...
        lessThan(maxMilliseconds),
        reason:
            '$lookupCount lookups in large catalog (500 objects) took ${stopwatch.elapsedMilliseconds}ms (expected < ${maxMilliseconds}ms)',
      );
    });
  });
}
