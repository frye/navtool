  ...
  group('S57 Geometry Stitching', () {
    ...
    test('should stitch edges without duplicate coordinates', () {
      ...
      expect(count, equals(1), 
             reason: 'Shared coordinate ${stitchCoord} should appear only once after stitching');
    });
    ...
    test('should stitch multiple edges in sequence', () {
      ...
      final pointers = [
        ...
        const S57SpatialPointer(refId: 3, isEdge: true, reverse: false), // (10,10) -> (0,10)
      ];
      ...
      final coords = geometry.rings.first;
      expect(coords.length, equals(4), reason: 'Should have 4 unique coordinates after stitching');
      
      // Verify coordinate sequence
      expect(coords[0].x, equals(0.0));  expect(coords[0].y, equals(0.0));   // Start of E1
      expect(coords[1].x, equals(10.0)); expect(coords[1].y, equals(0.0));   // End E1/Start E2 (shared)
      expect(coords[2].x, equals(10.0)); expect(coords[2].y, equals(10.0));  // End E2/Start E3 (shared)  
      expect(coords[3].x, equals(0.0));  expect(coords[3].y, equals(10.0));  // End of E3
    });
    ...
    test('should handle non-contiguous edges without stitching', () {
      ...
      final coords = geometry.rings.first;
      expect(coords.length, equals(4), reason: 'Should have all 4 coordinates when edges don\'t connect');
      
      // Verify coordinate sequence (no stitching should occur)
      expect(coords[0].x, equals(0.0));  expect(coords[0].y, equals(0.0));   // Start of E1
      expect(coords[1].x, equals(10.0)); expect(coords[1].y, equals(0.0));   // End of E1
      expect(coords[2].x, equals(10.0)); expect(coords[2].y, equals(10.0));  // Start of E3
      expect(coords[3].x, equals(0.0));  expect(coords[3].y, equals(10.0));  // End of E3
    });
    ...
    test('should stitch node and edge pointers', () {
      ...
      final pointers = [
        const S57SpatialPointer(refId: 1, isEdge: false, reverse: false), // Node 1: (0,0)
        const S57SpatialPointer(refId: 1, isEdge: true, reverse: false),  // Edge 1: (0,0) -> (10,0)
      ];
      ...
      final coords = geometry.rings.first;
      expect(coords.length, equals(2), reason: 'Node and edge start should be stitched');
      
      // Verify no duplication at (0,0)
      expect(coords[0].x, equals(0.0)); expect(coords[0].y, equals(0.0));   // Node/Edge start
      expect(coords[1].x, equals(10.0)); expect(coords[1].y, equals(0.0));  // Edge end
    });
    ...
    test('should handle empty coordinate chains gracefully', () {
      ...
      final pointers = [
        const S57SpatialPointer(refId: 1, isEdge: false, reverse: false), // Valid node
        const S57SpatialPointer(refId: 999, isEdge: true, reverse: false), // Missing edge
      ];
