...
void main() {
  group('S57 Geometry Degenerate Warning', () {
    ...
    test('should detect and skip degenerate edge with warning', () {
      ...
      expect(
        store.warnings.any((w) => w.contains('${degenerateEdge.id}')),
        isTrue,
      );
    ...
    test('should not flag valid edge as degenerate', () {
      ...
      const validEdge = S57Edge(
        id: 102,
        nodes: [S57Node(id: 1, x: 0.0, y: 0.0), S57Node(id: 2, x: 1.0, y: 1.0)],
      );
    ...
    test('should handle multiple degenerate edges in sequence', () {
      ...
      expect(
        store.warnings.any((w) => w.contains('Degenerate edge 201')),
        isTrue,
      );
      expect(
        store.warnings.any((w) => w.contains('Degenerate edge 202')),
        isTrue,
      );
    ...
    test('should continue with valid edges after skipping degenerate ones', () {
      ...
      // Assert
      expect(
        store.warnings.any((w) => w.contains('Degenerate edge 999')),
        isTrue,
      );
    ...
    test(
      'should provide helpful warning messages for different degenerate conditions',
      () {
        // Arrange
        final store = PrimitiveStore();
        ...
        expect(
          singleNodeWarnings.any(
            (w) => w.contains('Degenerate edge 2 with 1 nodes'),
          ),
          isTrue,
        );
        // Empty edge should either generate a warning or be handled by the "No valid coordinates" fallback
        expect(
          emptyEdgeWarnings.any(
            (w) =>
                w.contains('Degenerate edge 1') ||
                w.contains('No valid coordinates'),
          ),
          isTrue,
        );
      },
    ...
    test(
      'should maintain geometry integrity after skipping degenerate edges',
      () {
