  ...
  group('HTTP Client URL Construction Fix Tests', () {
    ...
    group('URL Construction Logic', () {
      ...
      test('should detect when path is already a full URL', () {
        ...
        expect(
          HttpClientService.isFullUrl('https://gis.charttools.noaa.gov/test'),
          isTrue,
        );
      ...
      test('should preserve query parameters for full URLs', () async {
        ...
        const fullUrl =
            'https://gis.charttools.noaa.gov/arcgis/rest/services/encdirect/enc_coverage/MapServer/0/query';
        final queryParams = {'where': '1=1', 'outFields': '*', 'f': 'json'};
      ...
      test('should work with NOAA catalog endpoint specifically', () async {
        ...
        await httpClientService.get(
          noaaCatalogEndpoint,
          queryParameters: {'where': '1=1', 'outFields': '*', 'f': 'json'},
        );
    ...
    group('Error Handling', () {
      ...
      test('should preserve original error handling behavior', () async {
        ...
        expect(
          ...
          throwsA(
            isA<Exception>(),
          ), // Now throws AppError instead of DioException
        );
      });
    });
    ...
    group('Integration with NOAA Endpoints', () {
      test('should fix the exact URL construction issue from the bug report', () {
        ...
        expect(
          brokenUrl,
          equals(
            'https://charts.noaa.govhttps://gis.charttools.noaa.gov/arcgis/rest/services/encdirect/enc_coverage/MapServer/0/query',
          ),
        );

        // The fixed behavior (what should happen with the fix)
        final isFullUrl = HttpClientService.isFullUrl(catalogEndpoint);
        final correctUrl = isFullUrl
            ? catalogEndpoint
            : baseUrl + catalogEndpoint;
