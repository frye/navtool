  ...
  group('NoaaErrorClassifier Tests', () {
      ...
      test('should identify non-retryable DioExceptions', () {
        ...
        for (final exception in nonRetryableDioExceptions) {
          expect(
            ...
            isFalse,
            reason:
                'Expected ${exception.type} with status ${exception.response?.statusCode} to be non-retryable',
          );
        }
      });
    ...
    group('getUserFriendlyMessage', () {
      test('should return user-friendly message for network connectivity', () {
        ...
        expect(
          message,
          'Unable to connect to NOAA services. Please check your internet connection and try again.',
        );
      });
      ...
      test('should return user-friendly message for rate limiting', () {
        ...
        expect(
          message,
          'Making too many requests to NOAA. Please wait a moment before trying again.',
        );
      });
      ...
      test('should return user-friendly message for chart not available', () {
        ...
        expect(
          message,
          'Chart US5CA52M is not currently available from NOAA. It may have been updated or removed.',
        );
      });
      ...
      test('should return user-friendly message for service unavailable', () {
        ...
        expect(
          message,
          'NOAA services are temporarily unavailable. This is usually due to maintenance. Please try again later.',
        );
      });
      ...
      test('should return user-friendly message for chart download failure', () {
        ...
        expect(
          message,
          'Failed to download chart US4FL11M. The download may have been interrupted or the file may be corrupted.',
        );
      });

      test('should return generic message for unknown NOAA exceptions', () {
        // Arrange
        final exception = NoaaApiException(
          'Unknown NOAA error',
        ...
        expect(
          message,
          'An error occurred while communicating with NOAA services. Please try again later.',
        );
      });
    ...
    group('getRecoveryRecommendation', () {
      test(
        'should provide recovery recommendation for network connectivity',
        () {
          // Arrange
          final exception = NetworkConnectivityException();

          // Act
          final recommendation = NoaaErrorClassifier.getRecoveryRecommendation(
            exception,
          );
      ...
      test(
        'should provide recovery recommendation for chart not available',
        () {
          // Arrange
          final exception = ChartNotAvailableException('US5CA52M');
      ...
      test(
        'should provide recovery recommendation for service unavailable',
        () {
          // Arrange
          final exception = NoaaServiceUnavailableException();
    ...
    group('classifyHttpError', () {
      test('should classify HTTP errors to appropriate NOAA exceptions', () {
        ...
        // 404 - Chart not found
        final notFoundException = NoaaErrorClassifier.classifyHttpError(
          404,
          'Chart not found',
          '/charts/US5CA52M',
        );
        ...
        final rateLimitException = NoaaErrorClassifier.classifyHttpError(
          429,
          'Too many requests',
        );
        ...
        final serviceException = NoaaErrorClassifier.classifyHttpError(
          503,
          'Service unavailable',
        );
        ...
        final serverException = NoaaErrorClassifier.classifyHttpError(
          500,
        ...
        final authException = NoaaErrorClassifier.classifyHttpError(
          401,
          'Unauthorized',
        );
