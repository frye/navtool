...
void expectNormalizedProgress(
  ...
  expect(
    progress.isFinite,
    isTrue,
    reason: reason ?? 'Progress must be finite',
  );
  expect(
...
void expectProgressCloseTo(
  double progress,
  double target, {
  double tolerance = 1e-6,
  String? reason,
}) => expectNormalizedProgress(
  progress,
  equals: target,
  tolerance: tolerance,
  reason: reason,
...
Iterable<double> expectAllNormalized(
...
Future<double> expectFirstNormalized(
