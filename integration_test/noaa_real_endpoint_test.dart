...
class _IntegrationTestLogger implements AppLogger {
  String _fmt(String level, String message, {String? context, Object? exception}) {
  ...
  @override
  void debug(String message, {String? context, Object? exception}) => debugPrint(_fmt('DEBUG', message, context: context, exception: exception));
  @override
  void info(String message, {String? context, Object? exception}) => debugPrint(_fmt('INFO', message, context: context, exception: exception));
  @override
  void warning(String message, {String? context, Object? exception}) => debugPrint(_fmt('WARN', message, context: context, exception: exception));
  @override
  void error(String message, {String? context, Object? exception}) => debugPrint(_fmt('ERROR', message, context: context, exception: exception));
  @override
...
void main() {
  ...
  group('NOAA Real Endpoint Integration Tests', () {
    ...
    final skipIntegrationTests = Platform.environment['SKIP_INTEGRATION_TESTS'] == 'true' || Platform.environment['CI'] == 'true';

    setUpAll(() {
      if (skipIntegrationTests) return;
      logger = _IntegrationTestLogger();
      httpClientService = HttpClientService(logger: logger)..configureNoaaEndpoints();
    ...
    tearDownAll(() { if (!skipIntegrationTests) httpClientService.dispose(); });
    group('Real API Connectivity', () {
      testWidgets('should successfully fetch NOAA chart catalog from real endpoint', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup') || 
            ...
            logger.warning('Network connectivity issue - expected in some test environments', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout fetching catalog - possible slow marine connection', context: 'Integration', exception: e);
          return;
        }
      });

      testWidgets('should handle filtered catalog requests correctly', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during filtered request', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during filtered catalog fetch', context: 'Integration', exception: e);
          return;
        }
      });

      testWidgets('should retrieve chart metadata for real charts', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during metadata fetch', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during metadata fetch', context: 'Integration', exception: e);
          return;
        }
      });
      ...
      testWidgets('should handle invalid chart IDs gracefully', (tester) async {
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during invalid chart test', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during invalid chart test', context: 'Integration', exception: e);
          return;
        }
      });
    });
    ...
    group('Chart Availability Checks', () {
      testWidgets('should check chart availability for real charts', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during availability check', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during availability check', context: 'Integration', exception: e);
          return;
        }
      });

      testWidgets('should return false for invalid chart availability', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during invalid availability check', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during invalid availability check', context: 'Integration', exception: e);
          return;
        }
      });
    });
    ...
    group('Error Handling and Resilience', () {
      testWidgets('should handle temporary network failures gracefully', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
      ...
      testWidgets('should handle slow marine connections with retries', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Marine connection simulation - network unavailable', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Marine connection timeout - simulating satellite internet conditions', context: 'Integration', exception: e);
          return;
        }
      });
    });
    ...
    group('Data Integrity Validation', () {
      testWidgets('should validate data structure integrity from real API', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (features.isNotEmpty) {
            ...
            if (e.message.contains('Failed host lookup')) {
          logger.warning('Network connectivity issue during data integrity test', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during data integrity test', context: 'Integration', exception: e);
          return;
        }
      });
    });
    ...
    group('API Schema Compatibility', () {
      testWidgets('should maintain compatibility with expected NOAA API schema', (tester) async {
        if (skipIntegrationTests) {
          logger.info('Skipping integration test - SKIP_INTEGRATION_TESTS is set', context: 'Integration');
          return;
        }
        ...
        try {
          ...
          if (e.message.contains('Failed host lookup')) {
            logger.warning('Network connectivity issue during schema test', context: 'Integration');
            return;
          }
          rethrow;
        } on TimeoutException catch (e) {
          logger.warning('Timeout during schema compatibility test', context: 'Integration', exception: e);
          return;
        }
      });
    });
