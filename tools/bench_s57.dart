...
Future<void> _benchmarkDatasetSize(int n) async {
  ...
  final results = {
    ...
    'build_performance': {
      ...
      'rtree_vs_linear_ratio': linearBuildMs > 0
          ? rtreeBuildMs / linearBuildMs
          : null,
    },
    'bounds_query_performance': {
      ...
      'speedup_factor':
          (linearBoundsStats['p50'] ?? 0) > 0 &&
              (rtreeBoundsStats['p50'] ?? 0) > 0
    ...
    'point_query_performance': {
      ...
      'speedup_factor':
          (linearPointStats['p50'] ?? 0) > 0 &&
              (rtreePointStats['p50'] ?? 0) > 0
          ? (linearPointStats['p50']! / rtreePointStats['p50']!)
          : null,
    },
    'performance_targets': {
      'bounds_p95_under_10ms':
          (rtreeBoundsStats['p95'] ?? double.infinity) < 10.0,
      'point_p95_under_5ms': (rtreePointStats['p95'] ?? double.infinity) < 5.0,
      'build_time_acceptable':
          rtreeBuildMs < (n / 10), // <100ms per 1k features rule of thumb
    },
  };
...
Map<String, dynamic> _generateFeatureAttributes(
  S57FeatureType type,
  Random random,
) {
  switch (type) {
    ...
    case S57FeatureType.buoy:
      return {
        'color': ['red', 'green', 'yellow'][random.nextInt(3)],
      };
    case S57FeatureType.lighthouse:
...
List<S57Bounds> _generateBoundsQueries(SpatialIndex index, int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    final sizeMultiplier =
        0.01 + random.nextDouble() * 0.2; // 1% to 20% of total bounds
...
List<Map<String, double>> _generatePointQueries(SpatialIndex index, int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    queries.add({'lat': lat, 'lon': lon, 'radius': radius});
  }
...
List<double> _benchmarkBoundsQueries(
  SpatialIndex index,
  List<S57Bounds> queries,
) {
  final times = <double>[];
...
List<double> _benchmarkPointQueries(
  SpatialIndex index,
  List<Map<String, double>> queries,
) {
  final times = <double>[];
