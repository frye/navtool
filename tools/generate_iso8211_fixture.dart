...
List<int> _createDDRRecord() {
  ...
  final recordLength =
      baseAddress + fieldAreaBytes.length + 1; // +1 for record terminator

  // Leader (24 bytes)
  final leader = BytesBuilder();
  leader.add(
    ascii.encode(recordLength.toString().padLeft(5, '0')),
  ); // positions 0-4
  ...
  leader.add(
    ascii.encode(baseAddress.toString().padLeft(5, '0')),
  ); // base address (positions 8-12)
...
List<int> _createDataRecordA() {
  ...
  final ft01Data = ascii.encode(
    'BCNCAR\u001f02\u001f01',
  ); // Beacon Cardinal + attributes
  ...
  directory.add(
    ascii.encode('00010'),
  ); // length (including subfield delimiters)
...
List<int> _createDataRecordB() {
  // Field data
  final ft02Data = ascii.encode(
    'SOUNDG\u001f150\u001f250',
  ); // Sounding + depth values
...
void _analyzeFixture(List<int> data) {
  ...
  while (offset < data.length) {
    ...
    final recordLengthStr = String.fromCharCodes(
      data.sublist(offset, offset + 5),
    );
    ...
    final leaderIdentifier = String.fromCharCodes(
      data.sublist(offset + 6, offset + 7),
    );
    final baseAddressStr = String.fromCharCodes(
      data.sublist(offset + 8, offset + 13),
    );
    final baseAddress = int.tryParse(baseAddressStr) ?? 0;

    print(
      'Record $recordNum: length=$recordLength, type=$leaderIdentifier, baseAddr=$baseAddress',
    );
