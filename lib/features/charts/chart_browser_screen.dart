...
class _ChartBrowserScreenState extends ConsumerState<ChartBrowserScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  children: [
                    if (active.isNotEmpty) _buildDownloadSection('Active', active),
                    if (queued.isNotEmpty) _buildDownloadSection('Queued', queued),
                    if (completed.isNotEmpty) _buildDownloadSection('Completed', completed),
                    if (failed.isNotEmpty) _buildDownloadSection('Failed', failed),
  ...
  Widget _buildDownloadSection(String title, List<DownloadProgress> items) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          ...items.map((d) => _buildDownloadRow(d)),
        ],
      ),
    );
  }

  Widget _buildDownloadRow(DownloadProgress progress) {
      ...
      trailing: SizedBox(
        ...
        child: Row(
          ...
          children: [
            ...
            _buildRowActions(progress, service, queueNotifier),
          ],
        ),
      ),
    );
  ...
  Widget _buildRowActions(DownloadProgress p, DownloadService service, DownloadQueueNotifier notifier) {
    ...
    if (p.status == DownloadStatus.failed) {
      return IconButton(
        ...
        onPressed: () {
          ...
          final resumeData = service.getResumeData(p.chartId);
