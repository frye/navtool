...
class PriorityStatus {
  ...
  int get totalRequestsInWindow =>
      criticalRequestsInWindow +
      highRequestsInWindow +
      normalRequestsInWindow +
      lowRequestsInWindow;
}
...
class PriorityRateLimiter extends RateLimiter {
  ...
  PriorityRateLimiter({super.requestsPerSecond, super.windowSize});
  ...
  void reserveCapacity(RequestPriority priority, int requests) {
    ...
    final totalReserved = _reservedCapacity.values.fold(
      0,
      (sum, val) => sum + val,
    );
  ...
  double _getPriorityMultiplier(RequestPriority priority) {
    switch (priority) {
      case RequestPriority.critical:
        return 0.25; // Wait 1/4 as long
      case RequestPriority.high:
        return 0.5; // Wait 1/2 as long
      case RequestPriority.normal:
        return 1.0; // Wait normal time
      case RequestPriority.low:
        return 1.5; // Wait 1.5x as long
    }
  }
