...
class SpatialOperations {
  ...
  static bool _isPolygonDegenerate(List<LatLng> polygon) {
    ...
    for (int i = nextIndex + 1; i < polygon.length; i++) {
      if (!_arePointsEqual(polygon[i], polygon[baseIndex], epsilon) &&
          !_arePointsCollinear(polygon[baseIndex], polygon[nextIndex], polygon[i], epsilon)) {
  ...
  static bool _arePointsCollinear(LatLng p1, LatLng p2, LatLng p3, double epsilon) {
  ...
  static bool _isPointOnSegment(LatLng point, LatLng vertex1, LatLng vertex2) {
    ...
    final crossProduct = (point.latitude - vertex1.latitude) * (vertex2.longitude - vertex1.longitude) -
                         (point.longitude - vertex1.longitude) * (vertex2.latitude - vertex1.latitude);
    ...
    final dotProduct = (point.longitude - vertex1.longitude) * (vertex2.longitude - vertex1.longitude) +
                       (point.latitude - vertex1.latitude) * (vertex2.latitude - vertex1.latitude);
    final squaredLength = (vertex2.longitude - vertex1.longitude) * (vertex2.longitude - vertex1.longitude) +
                          (vertex2.latitude - vertex1.latitude) * (vertex2.latitude - vertex1.latitude);
  ...
  static bool _rayIntersectsSegment(LatLng point, LatLng vertex1, LatLng vertex2) {
    ...
    final intersectionX = vertex1.longitude + 
  ...
  static bool _doPolygonEdgesIntersect(List<LatLng> polygon1, List<LatLng> polygon2) {
  ...
  static bool _lineSegmentsIntersect(LatLng p1, LatLng p2, LatLng q1, LatLng q2) {
  ...
  static int _orientation(LatLng p, LatLng q, LatLng r) {
    final val = (q.latitude - p.latitude) * (r.longitude - q.longitude) -
  ...
  static double calculateCoveragePercentage(List<LatLng> statePolygon, List<LatLng> chartPolygon) {
  ...
  static GeographicBounds? _getBoundsIntersection(GeographicBounds bounds1, GeographicBounds bounds2) {
