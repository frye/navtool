...
class CompressionServiceImpl implements CompressionService {
  ...
  Future<CompressionResult> compressChartData(
    ...
    try {
      _logger.info('Compressing chart data: $chartId (${data.length} bytes)', 
                   context: 'Compression');
      ...
      _logger.info('Chart compression completed: $chartId - ${result.toString()}',
                   context: 'Compression');
  ...
  Future<Uint8List> decompressChartData(
    ...
    try {
      _logger.debug('Decompressing chart data: $chartId (${compressedData.length} bytes)',
                    context: 'Compression');
      
      // Decompress GZIP data
      final decompressedData = GZipDecoder().decodeBytes(compressedData);
      
      _logger.debug('Chart decompression completed: $chartId (${decompressedData.length} bytes)',
                    context: 'Compression');
      
      return Uint8List.fromList(decompressedData);
    } catch (error) {
      _logger.error('Failed to decompress chart data: $chartId', 
                   exception: error);
      throw AppError.storage('Chart decompression failed', 
          originalError: error);
    }
  }
  ...
  Future<CompressionResult> compressChartDataWithSettings(
    ...
    try {
      _logger.info('Compressing chart data with custom settings: $chartId', 
                   context: 'Compression');
      ...
      _logger.error('Failed to compress chart data with settings: $chartId', 
                   exception: error);
      throw AppError.storage('Chart compression with settings failed', 
          originalError: error);
    }
  }
  ...
  Future<CompressionResult> compressRouteData(
    ...
    try {
      ...
      _logger.error('Failed to compress route data: $routeId', exception: error);
  ...
  Future<Uint8List> decompressRouteData(
    ...
    try {
      _logger.debug('Decompressing route data: $routeId', context: 'Compression');
      
      final decompressedData = GZipDecoder().decodeBytes(compressedData);
      
      return Uint8List.fromList(decompressedData);
    } catch (error) {
      _logger.error('Failed to decompress route data: $routeId', exception: error);
      throw AppError.storage('Route decompression failed', originalError: error);
    }
  }
  ...
  Future<CompressionResult> compressRoutesBackup(
    ...
    try {
      _logger.info('Creating compressed routes backup: $backupId', 
                   context: 'Compression');
      ...
      _logger.info('Routes backup compression completed: $backupId - ${result.toString()}',
                   context: 'Compression');
      
      return result;
    } catch (error) {
      _logger.error('Failed to compress routes backup: $backupId', exception: error);
      throw AppError.storage('Routes backup compression failed', originalError: error);
    }
  }
  ...
  Future<List<ExtractedFile>> extractChartArchive(
    ...
    try {
      _logger.info('Extracting chart archive: $chartId (${archiveData.length} bytes)',
                   context: 'Compression');
      ...
      if (_isZipArchive(archiveData)) {
        ...
        try {
          ...
          final file = ArchiveFile('data.bin', decompressed.length, decompressed);
      ...
      _logger.info('Chart archive extraction completed: $chartId (${extractedFiles.length} files)',
                   context: 'Compression');
      
      return extractedFiles;
    } catch (error) {
      _logger.error('Failed to extract chart archive: $chartId', exception: error);
      throw AppError.storage('Chart archive extraction failed', 
          originalError: error);
    }
  }
  ...
  Future<CompressionResult> compressCacheData(
    ...
    try {
      _logger.debug('Compressing cache data: $cacheKey', context: 'Compression');
      ...
      _logger.error('Failed to compress cache data: $cacheKey', exception: error);
  ...
  Future<Uint8List> decompressCacheData(
    ...
    try {
      _logger.debug('Decompressing cache data: $cacheKey', context: 'Compression');
      
      final decompressedData = GZipDecoder().decodeBytes(compressedData);
      
      return Uint8List.fromList(decompressedData);
    } catch (error) {
      _logger.error('Failed to decompress cache data: $cacheKey', exception: error);
      throw AppError.storage('Cache decompression failed', originalError: error);
    }
  }
  ...
  void _updateStats(String operation, CompressionResult result) {
    ...
    stats['totalOriginalSize'] = (stats['totalOriginalSize'] as int) + result.originalSize;
    stats['totalCompressedSize'] = (stats['totalCompressedSize'] as int) + result.compressedSize;
    stats['totalTime'] = (stats['totalTime'] as Duration) + result.compressionTime;
