...
class Iso8211Reader {
    ...
    while (_position < _data.lengthInBytes) {
      try {
        ...
        if (isFirstRecord) {
          ...
          _addWarning(Iso8211WarningCodes.subfieldParse, 
  ...
  Iso8211Record? _readSingleRecord(bool isDDR) {
    if (_position + _leaderSize > _data.lengthInBytes) {
      if (isDDR) {
        ...
        _addWarning(Iso8211WarningCodes.leaderLengthMismatch,
  ...
  List<Iso8211DirectoryEntry>? _parseDirectory(_LeaderInfo leader) {
    ...
    final directoryStart = _position - _leaderSize + _leaderSize; // After leader
    ...
    try {
      while (dirPos < directoryEnd) {
        ...
        final entrySize = leader.sizeOfFieldTag + leader.sizeOfFieldLength + leader.sizeOfFieldPosition;
        
        if (dirPos + entrySize > directoryEnd) {
          _addWarning(Iso8211WarningCodes.directoryTruncated,
        ...
        final length = _parseAsciiInt(_readAsciiAt(dirPos, leader.sizeOfFieldLength));
        dirPos += leader.sizeOfFieldLength;
        
        final position = _parseAsciiInt(_readAsciiAt(dirPos, leader.sizeOfFieldPosition));
  ...
  Map<String, List<int>> _extractFields(_LeaderInfo leader, List<Iso8211DirectoryEntry> directory) {
    ...
    for (final entry in directory) {
      try {
        ...
        if (fieldStart >= _data.lengthInBytes) {
          _addWarning(Iso8211WarningCodes.fieldBounds,
    ...
    while (_position + _leaderSize < _data.lengthInBytes) {
      try {
        ...
        if (potentialLength > _leaderSize && 
            ...
            potentialLength < 100000) { // Reasonable upper bound
          break;
        }
      } catch (e) {
        // Continue scanning
